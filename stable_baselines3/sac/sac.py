...
class SAC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            #print(replay_data.observations[0])
            #print("actions_pi : ",actions_pi)
            #print(actions_pi.shape)
            ...
            # ent_coef.requires_grad_()
            # actions_pi.requires_grad_()
            # min_qf_pi.requires_grad_()
            actor_loss.backward()
            #print(self.actor.optimizer.param_groups)

            #print(self.actor)
            #print("min_qf_pi fcn : ",min_qf_pi.grad_fn)
            #print("actor loss fcn : ",actor_loss.grad_fn)
            '''
            model = self.actor
            #print(model)
            ...
            lpw1 = list(model.parameters())[-6]
            lpw2 = list(model.parameters())[-4]
            muw = list(model.parameters())[-2]
            mub = list(model.parameters())[-1]
            #print(lpw1.grad)
            #exit()
            #print(len(muw.grad.detach().numpy()))
            #print(len(muw.detach().numpy()))
            ...
            # print(muw.shape)
            # output = hessian(actor_loss.grad_fn, muw)
            
            # print(output)
            # exit()
            grad = muw.grad.detach().numpy()[0]
            hes = np.gradient(muw.grad.detach().numpy()[0], muw.detach().numpy()[0])
            #print("grad : ",grad)
            #print("grad2 : ",grad2)
            #exit()
            #print("hessian : ",hes)
            #print("norm of mu gradient : ",np.linalg.norm(grad))
            #print("norm of mu hessian : ",np.linalg.norm(hes))
            #exit()
            self.grad_list.append(np.linalg.norm(grad))
            self.hes_list.append(np.linalg.norm(hes))
            #print("norm of lp gradient : ",np.linalg.norm(lpw1.grad.detach().numpy()),"   ",np.linalg.norm(lpw2.grad.detach().numpy()))
            '''
            #for p in model.parameters():
            #    print("grad p is ",p.grad[:10])
            
            #exit()
            # for name, param in model.named_parameters():
            #     print(f"Layer: {name} | Size: {param.size()} | Values : {param[:2]} \n")
            #     print(param.grad)
            #     #print(self.actor.mu.named_parameters())
            
            # print(log_prob.grad)
            # print(actions_pi.grad)
            # print(ent_coef.grad)
            # print(min_qf_pi.grad)
            
            #print("actor gradients : ",min_qf_pi.grad)
        ...
        self.logger.record("train/buffer_size", self.replay_buffer.pos, exclude="tensorboard")
